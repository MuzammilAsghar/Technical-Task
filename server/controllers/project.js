...
exports.createProject = AsyncHandler(async (req, res, next) => {
    ...
    const { projectName, description, liveUrl,strDate,gitHubLink, uploadImage, user } = req.body
    ...
    if (!strDate) return res.status(200).json({ success: false, message: "Start Date is required", });
    ...
    const project = new Project({ projectName, description,strDate, liveUrl, gitHubLink, uploadImage, user })
...
exports.updateProject = AsyncHandler(async (req, res, next) => {
    const { projectName, description, liveUrl,strDate, gitHubLink, uploadImage, id, status,user } = req.body
    if (!projectName) return res.status(200).json({ success: false, message: "Project Name is required", });
    if (!description) return res.status(200).json({ success: false, message: "Project Description is required", });
    if (!strDate) return res.status(200).json({ success: false, message: "Start Date is required", });
    if (!liveUrl) return res.status(200).json({ success: false, message: "Live Url link is required", });
    ...
    if (!user) return res.status(200).json({ success: false, message: "User Id is required", });
    console.log(strDate);
    const obj = req.file ? {uploadImage:req.file.path} : {true:true}
    const project = await Project.updateOne({ _id: id }, { $set: { projectName, description, strDate,liveUrl, gitHubLink, status,...obj } })
